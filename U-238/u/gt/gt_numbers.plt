...
test(known_seed, []) :-
    random_number([rand_state(2, Next_Seed), integer, range(0..10)], OptsOut, Num),
    assertion(Next_Seed == 2207042835),
    assertion(Num == 8),
    OptsOut / global_options / rand_options / rand_state ^= rand_state(S1, S2),
    assertion(S1 == Next_Seed),
    assertion(S2 =@= _).

test(random_number1_nondet, [L == [p(2, 1), p(4, 2), p(3, 3), p(1, 3)]]) :-
    findall(p(X, S), 
            random_number([generator(randgen:test_sequence1), rand_state(0, S), 
